...
# RL task
n_rl_trials = 40; # should be divided by 10
n_rl_blocks = 1;
n_rl_trials_per_block = n_rl_trials/n_rl_blocks;
rl_value_groups = c(40, 60); 
rl_value_sd = 20;
...
timing_RL = data.frame(sec = c(1000, 1500, 2000, 3000, 3500), reps = c(16, 10, 8, 3, 3)) # isi and iti timings, taken from an exponential distribution, aevraged to 1700 ms.
...
for (n in 1:n_designs){
  ...
  high_cat_on_left <- sample(rep(c(0,1), nrow(subset(novel_trials, left_category_value!=right_category_value))/2),replace=FALSE)
  ...
  for (b in 1:n_blocks){
    ...
    curr_novel_trials = sample_frac(novel_trials %>% subset(block==b))
    ...
    for (t in 1:nrow(curr_block)){
      ...
      # first three trials in every block are new trials.
  ...
  # remove test items from stimuli set
  ...
  practice_stims <- mutate(practice_stims,
                           category_value = c(20,20,20,40,40,60,60,60,80,80),
                           exemplar_value = c(0,20,40,40,60,40,60,80,80,100))
  
  # create decisions
  left_index = c(0, 8, 9, 2, 6);
  right_index = c(3, 5, 4, 7, 1);
  pair_type = c("20-80", "40-60", "60-80", "20-60","20-40");
...
for (n in 1:n_designs){
  ...
  # repeat pair types to create all trials
  ...
  # convert string sizes to numbers so we could change the position of stims according to high_size_on_left
  size_trials <- size_trials %>% mutate(
  ...
  # save data frame
  write.csv(size_trials,sprintf("../Size_judgement/Task/Design_matrix/size_trials_v%d.csv",n), row.names = FALSE, fileEncoding = "UTF-8")
...
for (n in 1:n_designs){
  ...
  # create two value distributions for stims
  rl_value_groups = sort(rl_value_groups)
  low_dist = sample(c(rep(rl_value_groups[1], n_rl_trials*rl_percent_mean),
                      rep(rl_value_groups[1]+rl_value_sd, n_rl_trials*(1-rl_percent_mean)/2),
                      rep(rl_value_groups[1]-rl_value_sd, n_rl_trials*(1-rl_percent_mean)/2)))
  high_dist = sample(c(rep(rl_value_groups[2], n_rl_trials*rl_percent_mean),
                      rep(rl_value_groups[2]+rl_value_sd, n_rl_trials*(1-rl_percent_mean)/2),
                      rep(rl_value_groups[2]-rl_value_sd, n_rl_trials*(1-rl_percent_mean)/2)))
  
  # add block info 
  blocks = rep(1:n_rl_blocks, n_rl_trials_per_block);
  ...
  for (b in 1:n_rl_blocks){
    ...
    # compute isi and iti
    curr_isi = sample(rep(timing_RL$sec, timing_size$reps), replace = FALSE)
    curr_iti = sample(rep(timing_RL$sec, timing_size$reps), replace = FALSE)
    iti = c(iti,curr_iti)
    isi = c(isi,curr_isi)
  ...
  high_item_on_left = sample(rep(c(0,1), n_rl_trials/2));
    
  # assign stims according to n_design, so as to counterbalance the stims across participants
  if (n %% n_designs %in% c(0,2)){
    high_stim = "A"; low_stim = "B"
  } else {
    high_stim = "B"; low_stim = "A"
  }
  ...
  low_stims <- data.frame(img = low_stim, 
                          ...
                          item_value = low_dist)
  # create data frame
  rl_trials <- data.frame()
  for (t in 1:n_rl_trials){
    if (high_item_on_left[t] == 1){
      curr_left_stim = high_stims[t,]; curr_right_stim = low_stims[t,];
    } else {
      curr_left_stim = low_stims[t,]; curr_right_stim = high_stims[t,];
    }
    rl_trials[t, "trial_num"] = t-1;
    rl_trials[t, "is_practice"] = 0;
    rl_trials[t, "block"] = blocks[t];
    rl_trials[t, "iti"] = iti[t];
    rl_trials[t, "isi"] = isi[t];
    rl_trials[t, "high_group_on_left"] = high_item_on_left[t];
    rl_trials[t, "left_group_value"] = curr_left_stim$group_value;
    rl_trials[t, "right_group_value"] = curr_right_stim$group_value;
    rl_trials[t, "left_item_value"] = curr_left_stim$item_value;
    rl_trials[t, "right_item_value"] = curr_right_stim$item_value;
    rl_trials[t, "left_img"] = curr_left_stim$img;
    rl_trials[t, "right_img"] = curr_right_stim$img;
    rl_trials[t, "left_img_path"] = paste0("Stimuli/Fractals_stims/",curr_left_stim$img,".jpg");
    rl_trials[t, "right_img_path"] = paste0("Stimuli/Fractals_stims/",curr_right_stim$img,".jpg");
  }

  # save data frame
  write.csv(rl_trials,sprintf("../Simple_RL/Task/Design_matrix/RL_trials_v%d.csv",n), row.names = FALSE, fileEncoding = "UTF-8")
